...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId) {
        ...
        //step3：List<Cart> --> List<OrederItem>
        ...
        return ServerResponse.serverResponsebBySuccess(orderVO);
    }
    ...
    public ServerResponse get_order_cart_product(Integer userId) {
        ...
        //step2：List<Cart> --> List<OrederItem>
        ServerResponse serverResponse = getCartOrderItem(userId,cartList);
        if(!serverResponse.isScuess()){
            return serverResponse;
        }
        ...
        //step4：返回结果
        return ServerResponse.serverResponsebBySuccess(cartOrderItemVO);
    }
    ...
    public ServerResponse list(Integer userId,Integer pageNum,Integer pageSize) {
        ...
        return ServerResponse.serverResponsebBySuccess(pageInfo);
    }
    ...
    public ServerResponse send_goods(Long orderNo) {
        ...
        int result = orderMapper.updateByPrimaryKey(order);
        if(result > 0){
            return ServerResponse.serverResponsebBySuccess("发货成功");
        }
     ...
     * 取消订单后增加库存
     * */
    private void increaseProductStock(Long orderNo){
    ...
    private ServerResponse getCartOrderItem(Integer userId, List<Cart> cartList){
        ...
        return  ServerResponse.serverResponsebBySuccess(orderItemList);
    }
    ...
    public ServerResponse query_order_pay_status(Long orderNo) {
        ...
        if(order.getStatus() == Const.OrderStatusEnum.ORDER_PAYED.getCode()){
            return ServerResponse.serverResponsebBySuccess(true);
        }

        return ServerResponse.serverResponsebBySuccess(false);
    }
            ...
            for(Order order : orderList){
                ...
                if(orderItemList != null && orderItemList.size() > 0){
                    for(OrderItem orderItem : orderItemList){
                        ...
                        productMapper.updateProductKeySelectvie(product);
                    }
                }
                //关闭订单
                order.setStatus(Const.OrderStatusEnum.ORDER_CANCELED.getCode());
...
//                .setNotifyUrl( "http://382agm.natappfree.cc/shopping/order/alipay_callback.do")
                .setNotifyUrl( "http://39.105.65.2:8080/Shopping/order/alipay_callback.do")
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
//                String filePath = String.format("D:/ftpfile/qr-%s.png", response.getOutTradeNo());
                String filePath = String.format("/ftpfile/img/qr-%s.png", response.getOutTradeNo());
                ...
                return ServerResponse.serverResponsebBySuccess(map);
